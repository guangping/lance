...
//扩展验证 手机号  电话
$.Datatype.mobile = /^(13[0-9]|15[0-9]|18[0-9]|17[0-9])\d{8}$/;
$.Datatype.tel = /^0\d{2,3}-?\d{7,8}$/;
$.Datatype.numberZero = /^[0-9]*[1-9][0-9]*$/;
//中文
$.extend($.Datatype, {
    "z1-200": /^[\u4E00-\u9FA5\uf900-\ufa2d]{1,200}$/
});
$.extend($.Datatype, {
    "china1-15": /^[\u4E00-\u9FA5\uf900-\ufa2d]{1,15}$/
});
$.extend($.Datatype, {
    "*china200": function (gets, obj, curform, regxp) {
        return ServiceUtils.strLen(gets) > 200 ? false : true;
    }
});
$.extend($.Datatype, {
    "numberZero30": function (gets, obj, curform, regxp) {
        var reg = /^[0-9]*[1-9][0-9]*$/;
        if (reg.test($(obj).val())) {
            if ($(obj).val() > 295000) {
                return false;
...
$.extend($.Datatype, {
    "*address": function (gets, obj, curform, regxp) {
        var reg = /[\w\W]+/;
        var val = $(obj).val();
        if (val.length == 0) {
            return false;
        ...
        var sign = true;
        var data = $(obj).siblings('input[type="hidden"]');
