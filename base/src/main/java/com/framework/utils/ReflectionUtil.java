...
public class ReflectionUtil {
    ...
    public static Map<String,Object>  getRealMap(Object args){
        Map rval=JSONObject.parseObject(JSONObject.toJSONString(args),Map.class);
        Field[] fieldses = args.getClass().getDeclaredFields();
        for (Field obj : fieldses) {
            if (null != obj.getAnnotation(AutoPK.class) || null != obj.getAnnotation(NotDbField.class)) {
               rval.remove(obj.getName());
            }
        }
        return rval;
    }
        ...
        for (Method m : ms) {
            ...
            if (name.startsWith("get") || name.startsWith("is")) {
                ...
                try {
                    ...
                    if (parent.getAnnotation(AutoPK.class) != null) {
                        poMap.remove(getFieldName(name));
                        continue;
                    }
                } catch (Exception e) {
                ...
                if (m.getAnnotation(AutoPK.class) != null) {
                    poMap.remove(getFieldName(name));
                }
            }
        }
