...
/*    */ {
/* 51 */   private static String SELECT_CORE_B_BF_POINTCUT = "select a.pointcut_id,a.class_path,a.comments from CORE_B_POINTCUT a ,CORE_B_SERVICE_POINTCUT b where a.pointcut_id=b.bf_point_cut_id and a.status_cd='00A' and b.busi_module_code=?";
/* 52 */   private static String SELECT_CORE_B_AF_POINTCUT = "select a.pointcut_id,a.class_path,a.comments from CORE_B_POINTCUT a ,CORE_B_SERVICE_POINTCUT b  where a.pointcut_id=b.af_point_cut_id and a.status_cd='00A' and b.busi_module_code=?";
/* 53 */   private static String INSERT_LOG_RECORED_TABLE = "insert into LOG_RECORED_TABLE(LOG_ID,LOG_TYPE_ID,LOG_LEVEL,OP_ACTION,LOG_NAME,COMMENTS,IN_PARAMS,OUT_PARAMS,CREATOR,system_flag,CREATE_DATE) values(SEQ_LOG_RECORED_TABLE_LOG_ID.NEXTVAL,?,?,?,?,?,?,?,?,?,SYSDATE)";
/*    */ 
/* 55 */   private static String SELECT_LOG_RECORD_TABLE = "SELECT LOG_ID,LOG_TYPE_ID,LOG_LEVEL,OP_ACTION,LOG_NAME,COMMENTS,IN_PARAMS,OUT_PARAMS,CREATOR,CREATE_DATE FROM LOG_RECORED_TABLE WHERE 1=1 ";
/*    */ 
/* 57 */   private static String SELECT_ATOM_L_REGISTER = "SELECT a.* FROM ATOM_L_REGISTER  a where a.SERVICE_CODE=? and  a.ATOM_CODE=? ";
/* 58 */   private static String SELECT_AN_QUERY_CONDITION = "SELECT a.* FROM AN_QUERY_CONDITION a where a.query_code=?";
/* 59 */   private static String SELECT_AN_QUERY_RESULT = "SELECT a.* FROM AN_QUERY_RESULT a where a.query_code=?";
/* 60 */   private static String SELECT_AN_QUERY_RESULT_FIELD = "SELECT a.* FROM AN_QUERY_RESULT_FIELD a where a.query_code=?";
/* 61 */   private static String SELECT_AN_QUERY_RESULT_GRAPH = "SELECT a.* FROM AN_QUERY_RESULT_GRAPH a where a.query_code=?";
...
/* 93 */   private static String SELECT_PAGE_P_ITEM_REL_BUS = "select t.*,p.busi_module_code from  PAGE_P_ITEM  t ,PAGE_MODUL_REL_SERVICE p where t.page_item_id=p.page_item_id and page_id=? order by t.order_id";
/*    */ 
/* 95 */   private static String SELECT_PAGE_MODULE_REL_SERVICE = "select a.* from PAGE_MODUL_REL_SERVICE a,CORE_B_SERVICE b   where a.busi_module_code=b.busi_module_code and  a.page_item_id in(select page_item_id from PAGE_P_ITEM where page_id=?)";
/* 96 */   private static String SELECT_CORE_B_SERVICE_REL_ATOM = "select t.* from CORE_B_SERVICE_REL_ATOM a,CORE_B_SERVICE b,ATOM_L_REGISTER t where a.busi_module_code=b.busi_module_code and a.atom_reg_id=t.atom_reg_id and a.busi_module_code=?";
/*    */ 
/* 98 */   private static String SLEECT_PAGE_P_ITEM = "select a.page_id, a.page_item_id page_item_code, a.page_m_code, a.status_cd ,a.order_id, a.item_tag,a.col1, a.col2, a.col3, a.page_url, a.load_type,a.page_item from PAGE_P_ITEM a where page_id = ? order by a.order_id";
/* 99 */   private static String SLEECT_PAGE_ITEM_BY_CODE = "select a.page_id, a.page_item_id page_item_code, a.page_m_code, a.status_cd ,a.order_id, a.item_tag,a.col1, a.col2, a.col3, a.page_url, a.load_type,a.page_item from PAGE_P_ITEM a where page_item_id = ?";
...
/*    */   public static final String INSERT_CLIENT_CALLLOG = "insert into inf_comm_client_calllog( req_time, rsp_time, op_code, ep_address, req_xml, rsp_xml, state, state_msg,param_desc, result_desc,col1,col2,col3,col4,col5,log_id) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
/*    */   public static final String SELECT_REQUEST_USER = "select * from inf_comm_client_request_user where user_id=? ";
/*    */   public static final String INSERT_INF_CRM_SPS_LOG = " INSERT INTO inf_crm_sps_log a (order_item_id,transaction_id,service_code,action,create_date,content)VALUES (?,?,?,?,sysdate,?)";
...
/*    */   {
/* 38 */     return SingletonHolder.sqlsManager;
/*    */   }
/*    */ 
/*    */   public F_SQLS()
/*    */   {
/* 43 */     SqlUtil.initSqls(F_SQLS.class, this, this.sqls);
/*    */   }
/*    */ 
/*    */   public void main(String[] args) {
/* 47 */     Sqls f = getInstance();
/*    */   }
/*    */ 
/*    */   static class SingletonHolder
/*    */   {
/* 12 */     static Sqls sqlsManager = new F_SQLS();
/*    */ 
/*    */     static {
/*    */       try { Map mysqls = sqlsManager.sqls;
/*    */ 
/* 27 */         Sqls f_sql_local = (Sqls)PlatService.getPlatClass(sqlsManager.getClass());
/* 28 */         Class f_sql_local_class = f_sql_local.getClass();
/* 29 */         SqlUtil.initSqls(f_sql_local_class, f_sql_local, mysqls);
/*    */       } catch (Exception ex) {
 ...
 * Qualified Name:     com.ztesoft.framework.sqls.F_SQLS
