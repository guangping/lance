...
public class FieldDictionary {
	...
	public Field field(Class cls, String name, Class definedIn) {
		...
		Field field = (Field) fields
				.get(definedIn != null ? (Object) new FieldKey(name, definedIn,
						0) : (Object) name);
		if (field == null) {
			throw new ObjectAccessException("No such field " + cls.getName()
					+ "." + name);
		} else {
			return field;
		}
	...
	private Map buildMap(final Class type, boolean tupleKeyed) {
			...
			if (!keyedByFieldNameCache.containsKey(type)) {
				...
				for (final Iterator iter = superClasses.iterator(); iter
					...
					if (!keyedByFieldNameCache.containsKey(cls)) {
						...
						keyedByFieldKeyCache.put(cls,
								sorter.sort(type, keyedByFieldKey));
					}
		...
		return (Map) (tupleKeyed ? keyedByFieldKeyCache.get(type)
