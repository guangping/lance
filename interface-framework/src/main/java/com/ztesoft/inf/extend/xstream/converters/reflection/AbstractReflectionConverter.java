...
public abstract class AbstractReflectionConverter implements Converter {
	...
	public void marshal(Object original, final HierarchicalStreamWriter writer,
		...
		final Object source = serializationMethodInvoker
				.callWriteReplace(original);
		Mapper mapper = context.getMapper();
		if (source.getClass() != original.getClass()) {
			String attributeName = mapper
					.aliasForSystemAttribute("resolves-to");
		...
		reflectionProvider.visitSerializableFields(source,
				new ReflectionProvider.Visitor() {
						...
						if (!seenFields.contains(fieldName) && newObj != null) {
							...
							if (mapping != null) {
								...
								String nodeName = mapper
										.getImplicitCollectionItemNameByPath(path);
								Class itemType = mapper.realClassByPath(path);
								tracker.popElement();
								if (nodeName != null) {
									Collection list = (Collection) newObj;
									for (Iterator iter = list.iterator(); iter
											.hasNext();) {
										Object obj = iter.next();
										writeField(fieldName, nodeName,
												itemType, definedIn, obj);
									}
								} else {
									tracker.pushElement(fieldName);
									nodeName = mapper
											.getAliasNameByPath(context
													.getPathTracker().getPath()
													.toString());
									tracker.popElement();
									writeField(fieldName, nodeName, fieldType,
											definedIn, newObj);
								}
							}
						}
					...
					private void writeField(String fieldName, String aliasName,
						...
						String nodeName = aliasName != null ? aliasName
								: mapper.serializedMember(source.getClass(),
										fieldName);
						ExtendedHierarchicalStreamWriterHelper.startNode(
								writer, nodeName, fieldType);
						if (newObj != null) {
							...
							// if
							// (!serializedClassName.equals(mapper.serializedClass(defaultType)))
							// {
	...
	protected void marshallField(final MarshallingContext context,
		...
		context.convertAnother(
				newObj,
				mapper.getLocalConverter(field.getDeclaringClass(),
						field.getName()));
	}
	...
	public Object doUnmarshal(final Object result,
		...
		while (it.hasNext()) {
			...
			if (fieldExistsInClass) {
				...
				SingleValueConverter converter = mapper
						.getConverterFromAttribute(field.getDeclaringClass(),
		...
		while (reader.hasMoreChildren()) {
			...
			String fieldName = mapper.realMember(result.getClass(),
					originalNodeName);
			Mapper.ImplicitCollectionMapping implicitCollectionMapping = mapper
					.getImplicitCollectionDefForFieldName(result.getClass(),
							fieldName);
			Class classDefiningField = determineWhichClassDefinesField(reader,
					mapper);
			boolean fieldExistsInClass = implicitCollectionMapping == null
					&& reflectionProvider.fieldDefinedInClass(fieldName,
							result.getClass());
			Class type = implicitCollectionMapping == null ? determineType(
					reader, fieldExistsInClass, result, fieldName,
					classDefiningField, context) : implicitCollectionMapping
					.getItemType();
			final Object value;
			if (fieldExistsInClass) {
				Field field = reflectionProvider.getField(
				...
				// TODO the reflection provider should have returned the proper
				...
				value = type != null ? context.convertAnother(result, type)
						: null;
			}
			...
			if (fieldExistsInClass) {
				...
				implicitCollectionsForCurrentObject = writeValueToImplicitCollection(
						context, value, implicitCollectionsForCurrentObject,
						result, originalNodeName);
			}
			reader.moveUp();
		}
		return result;
	...
	private Map writeValueToImplicitCollection(UnmarshallingContext context,
			Object value, Map _implicitCollections, Object result,
			String itemFieldName) {
		final Mapper mapper = context.getMapper();
		Map implicitCollections = _implicitCollections;
		...
		if (fieldName == null) {
			fieldName = mapper.getColFieldNameByPath(context.getPathTracker()
					.getPath().toString());
		}
		if (fieldName != null) {
			...
			if (collection == null) {
				Class fieldType = mapper
						.defaultImplementationOf(reflectionProvider
				...
				reflectionProvider.writeField(result, fieldName, collection,
						null);
	...
	protected Object instantiateNewInstance(HierarchicalStreamReader reader,
		...
		if (currentObject != null) {
			...
			return reflectionProvider.newInstance(mapper
					.realClass(readResolveValue));
		} else {
	...
	private Class determineType(HierarchicalStreamReader reader,
			...
			Class _definedInCls, UnmarshallingContext context) {
		final Mapper mapper = context.getMapper();
		Class definedInCls = _definedInCls;
		...
		if (classAttribute != null) {
			...
			if (itemType != null) {
				...
				itemType = HierarchicalStreams
						.readClassByXPath(mapper, context);
				if (itemType != null)
					return itemType;
				String originalNodeName = reader.getNodeName();
				if (definedInCls == null) {
				...
				return HierarchicalStreams.readClassType(originalNodeName,
						reader, mapper);
			}
		} else {
